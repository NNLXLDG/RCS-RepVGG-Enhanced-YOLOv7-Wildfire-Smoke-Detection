...
class GhostConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        super().__init__()
        c_ = c2 // 2
        self.conv1 = Conv(c1, c_, k, s, g=1, act=act)
        self.conv2 = Conv(c_, c_, 5, 1, g=c_, act=act)
    def forward(self, x):
        x1 = self.conv1(x)
        x2 = self.conv2(x1)
        return torch.cat([x1, x2], dim=1)

class GhostRepVGG(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, act=True):
        super().__init__()
        self.conv = GhostConv(in_channels, out_channels, k=kernel_size, s=stride, act=act)
...
class GhostSEBlock(SEBlock):
    # 直接继承 SEBlock，无需修改
    pass

class GhostSR(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        c1_ = c1 // 2
        c2_ = c2 // 2
        self.repconv = GhostRepVGG(c1_, c2_)
    def forward(self, x):
...
class GhostResSR(nn.Module):
    def __init__(self, c, shortcut=False):
        super().__init__()
        self.repconv1 = GhostRepVGG(c, c)
        self.repconv2 = GhostRepVGG(c, c)
        self.shortcut = shortcut
    def forward(self, x):
        return (x + self.repconv2(self.repconv1(x))) if self.shortcut else self.repconv2(self.repconv1(x))

class GhostRCSOSA(nn.Module):
    def __init__(self, c1, c2, n=1, se=False, e=0.5, stackrep=True):
        super().__init__()
        n_ = n // 2
        c_ = make_divisible(int(c1 * e), 8)
        self.conv1 = GhostRepVGG(c1, c_)
        self.conv3 = GhostRepVGG(int(c_ * 3), c2)
        self.sr1 = nn.Sequential(*[GhostSR(c_, c_) for _ in range(n_)])
        self.sr2 = nn.Sequential(*[GhostSR(c_, c_) for _ in range(n_)])
        self.se = GhostSEBlock(c2) if se else None
    def forward(self, x):
        x1 = self.conv1(x)
        x2 = self.sr1(x1)
        x3 = self.sr2(x2)
