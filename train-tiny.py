...
def print_training_info(save_dir, model_cfg, dataset_cfg, epochs, batch_size):
    ...
    separator = "━" * 80
    logger.info(f"\n{separator}")
    logger.info(f"🔥 YOLOv7-Tiny Fire/Smoke Detection Model Training - Starting")
    ...
    logger.info(f"{separator}")
    logger.info(f"🚀 Starting Tiny-based training... Please wait for model convergence")
    logger.info(f"{separator}\n")


def train(hyp, opt, device, tb_writer=None):
    """
    主训练函数 - Tiny版本，优化适配M系列芯片
    ...
    if rank in [-1, 0]:
        ...
        logger.info(f'🎯 Tiny-based YOLOv7 training completed! Results saved to {colorstr("bold", save_dir)}')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='YOLOv7-Tiny Training Script - Optimized for Apple Silicon Mac and CPU training')
