...
这里我已经对数据路径进行替换，workers改为2，batchsize改为原来1/4（不然会爆内存）
...
```
python train.py --workers 2 --device 0 --batch-size 8 --data datasets_smokefire\data.yaml --img 640 640 --cfg cfg/training/rcs-yolo.yaml --weights '' --name rcs-yolo --hyp data/hyp_training.yaml
```

###### Multiple GPU training

```
python -m torch.distributed.launch --nproc_per_node 4 --master_port 9527 train.py --workers 2 --device 0,1,2,3 --sync-bn --batch-size 32 --data datasets_smokefire\data.yaml --img 640 640 --cfg cfg/training/rcs-yolo.yaml --weights '' --name rcs-yolo --hyp data/hyp_training.yaml
```
